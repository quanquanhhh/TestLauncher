...
// using UnityEngine.Purchasing.Extension;  // 引入 FailedOrder, PendingOrder
...
// { 
//     public class IAPManager2 : SingletonComponent<IAPManager>, IIAPCallback
//     {
...
//
//         public void RestorePurchases(bool bClick)
//         {
...
//             // return;
// #endif
//             if (ClickToRestore) UIModule.Instance.ShowAsync<UIMaskLoading>();
//
//             if (!IsInitialized)
//             {
//                 Debug.LogError("IAPManager not initialized");
...
//
//             // 订阅 IAP 5.x 事件（关键：获取详细失败信息）
//             storeController.OnPurchaseFailed += OnPurchaseFailedEvent;
//             storeController.OnPurchasePending += OnPurchasePendingEvent;
//
//             isInitialized = true;
//             isInitializing = false;
//             FetchProducts();
...
//
//         // ========== IAP 5.x 事件处理方法 ==========
//         private void OnPurchaseFailedEvent(FailedOrder failedOrder)
//         {
//             var product = failedOrder.Product;
//             var reason = failedOrder.FailureReason;
//             var details = failedOrder.Details;   // 这里就是 Google Play 返回的原始错误信息
//
//             Debug.LogError($"OnPurchaseFailedEvent - Product: {product?.definition?.id}, Reason: {reason}, Details: {details}");
//
//             HandlePurchaseFailed(product, reason, details, "EventOnPurchaseFailed");
//         }
//
//         private void OnPurchasePendingEvent(PendingOrder pendingOrder)
//         {
//             Debug.Log($"Purchase pending for product: {pendingOrder.Product?.definition?.id}");
//             // 可以显示等待提示，例如 UIModule.Instance.Close<UIMaskLoading>(); 但保持等待
//         }
//         // =====================================
//
...
//         }
//
//         public void Purchase(string sProduct, Action claimAction, string adjustkey, int price, string name)
//         {
// #if UNITY_EDITOR
//             claimAction?.Invoke();
//             return;
...
//                 AdjustManager.Instance.LogAdjustRevenue(sAdjustKey, sPrice, product.metadata.isoCurrencyCode);
//                 TBAMgr.Instance.SendLogPurchase(sPrice, sProductId);
